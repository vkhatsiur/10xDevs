...
export const PUT: APIRoute = async ({ params, request, locals }) => {
  try {
    ...
    if (!front || !back) {
      return new Response(
        JSON.stringify({ error: 'Both front and back are required' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!updatedFlashcard) {
      return new Response(
        JSON.stringify({ error: 'Flashcard not found or access denied' }),
        { status: 404, headers: { 'Content-Type': 'application/json' } }
      );
    }

    return new Response(
      JSON.stringify({ flashcard: updatedFlashcard }),
